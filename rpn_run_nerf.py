...
def write_images_with_metrics(images, mean_metrics, far, args, with_test_time_optimization=False):
    if args.task != 'render_train_depth':
        result_dir = os.path.join(args.ckpt_dir, args.expname, "test_images_" + 
            ("with_optimization_" if with_test_time_optimization else "") + args.scene_id)
    else:
        result_dir = os.path.join(args.ckpt_dir, args.expname, "train_depth_" + args.scene_id)
...
def train_nerf(images, depths, valid_depths, poses, intrinsics, i_split, args, scene_sample_params, lpips_alex, gt_depths, gt_valid_depths):
    ...
    # optimize nerf
    print('Begin')
    N_iters = 200000 + 1  # TODO: switch back to 500k?
    global_step = start
    start = start + 1
...
def config_parser():
    ...
    parser.add_argument('task', type=str, help='one out of: "train", "test", "test_with_opt", "video", "extract"')
    ...
    parser.add_argument('--bbox_json', type=str, default="",)
...
def run_nerf():
    ...
    if "test" in args.task: 
        ...
        mean_metrics_test, images_train = render_images_with_metrics(
            None, i_train, images, depths, valid_depths, poses, H, W, intrinsics, lpips_alex, args,
            render_kwargs_test, with_test_time_optimization=with_test_time_optimization)
        write_images_with_metrics(images_train, mean_metrics_test, far, args, 
        ...
        rgbsigma, res, bbox_min, bbox_max = extract_nerf(torch.Tensor(poses[i_train]).to(device), args, render_kwargs_test)
        ...
        np.savez_compressed(output_path, rgbsigma = rgbsigma, resolution=res,
                            ...
                            from_mitsuba=False, from_ddp_nerf=True)
...
def extract_nerf(poses, args, render_kwargs_test):
    ...
    for i in tqdm(range(len(poses))):
        ...
        rgb = torch.sigmoid(raw[..., :3])  # consistent with data from instant ngp, density left unnormalized
        sigma = raw[..., 3].squeeze()
...
def get_scene_bounding_box(bbox_json):
    print("Using hardcoded bounding scene extent bbox.")
    return torch.Tensor([-9.8950, -8.5592, -3.2289]), torch.Tensor([ 8.1965, 10.1358,  8.2913]) # TODO: don't hardcode this
